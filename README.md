...
给 GNOME 快捷面板顶部那行「系统栏」（左边电池、右边一排图标）做自定义的小扩展（GNOME Shell 50 / Ubuntu 26.04 上开发测试）：
...
在系统栏的任意图标上**按住左键约 0.5 秒**即可进入编辑态：

- 整行出现橙色外框，行尾多出 `＋` 和 `✓` 两个按钮；
- **拖动图标**调整顺序（松手即保存到配置）；
- **往上拖出这一行**再松手 = 从系统栏移除该图标；
- 点 `＋` 打开设置窗口（增删图标 / 自定义命令按钮），点 `✓` 或点面板外退出；
- 编辑态里点击图标**不会**触发原动作，退出后恢复正常。

> 实现说明：为了让“长按”不误触发截图/关机等动作，这一行的按钮点击由本扩展统一接管
> （短按 = 原动作，长按 = 编辑态），所以点击是扩展自己派发的，不依赖 shell 内部的 `clicked`。
